...
#Local imports
from TDEED.util.score import compute_mAPs
from TDEED.util.io import store_json, store_json_sn, store_json_snb
from TDEED.util.dataset import custom_collate_fn
...
def process_frame_predictions(dataset, classes, pred_dict, high_recall_score_threshold=0.01):
    ...
    fps_dict = {}
    for video, _, _, fps in dataset.videos:
        fps_dict[video] = fps
    ...
    pred_scores = {}
    h = 0
    # Itterate for each existed label
    for video, sub in (sorted(pred_dict.items())):
        # Itterate for each existed specific object
        events = []
        events_high_recall = []
        for sub_key, (scores, support) in (sub.items()):
            label = dataset.get_labels(video, sub_key)
            if np.min(support) == 0:
                support[support == 0] = 1
...
def evaluate(model, dataset, split, classes, save_pred=None, printed = True, 
    ...
    for video, sub_name, video_len, _ in dataset.videos:
        pred_dict[video][sub_name] = (
    ...
    for clip in tqdm(DataLoader(
        ...
        if batch_size > 1:
            ...
            _, batch_pred_scores = model.predict(clip['frame_close'], clip['frame_wide'], clip['track'])

            for i in range(clip['frame_close'].shape[0]):
                video = clip['video'][i]
                sub = clip['sub'][i]
                scores, support = pred_dict[video][sub]
            ...
            # Batched by dataset
            scores, support = pred_dict[clip['video'][0]][clip['sub'][0]]

            start = clip['start'][0].item()
            _, pred_scores = model.predict(clip['frame_close'], clip['frame_wide'], clip['track'])
            if start < 0:
                pred_scores = pred_scores[:, -start:, :]
                start = 0
            ...
            for i in range(1):
                start = clip['start'][0].item()
                _, pred_scores = model.predict(clip['frame_close'], clip['frame_wide'], clip['track'], augment_inference = True)
                if start < 0:
                    pred_scores = pred_scores[:, -start:, :]
                    start = 0
                end = start + pred_scores.shape[1]
