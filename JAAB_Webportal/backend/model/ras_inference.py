...
def track_glove(frames, p_model,b_model, tracker, clip_len):
    ...
    for idx, buffered_frame in enumerate(frames):     
        ...
        for obj_box in object_boxes:
            ...
            for i, pose_box in enumerate(pose_boxes):
                ...
                if iou > max_iou and iou >= 0.5:
                    max_iou = iou
                    best_match_keypoints = pose_keypoints_list[i]
                    best_match_confidences = pose_confidences_list[i]
        ...
        if detections:
            if len(detections) < 3 and detections[0] and detections[1]:
...
def draw_events(frames, pred_events, track):
    ...
    for idx, frame in enumerate(ann_frames):
        for i in range(track.shape[0]):
            _box_x, _box_y = track[i][idx][0], track[i][idx][1]
            cv2.circle(frame, (int(_box_x), int(_box_y)), 3, (0, 255, 0), 2)
...
def model_inference(args):
    ...
    tracker = Tracker(distance_function=euclidean_distance, distance_threshold=200)
