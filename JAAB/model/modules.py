...
class CrossAttentionLayer(nn.Module):
    def __init__(
        ...
        self._attention = nn.MultiheadAttention(embed_dim, num_heads)
        self._norm = LayerNorm(embed_dim)
    ...
    def forward(self, x_c, x_f, pos_enc):
        _queries = x_c + pos_enc
        _keys = x_f + pos_enc
        _values = x_f

        _att, _ = self._attention(_queries, _keys, _values)
        _att = _queries + self._dropout(_att)
        _att_norm = self._norm((_att).permute(0, 2, 1)).permute(0, 2, 1)
        _att = self._mlp(_att_norm) + _att
        return _att
...
class CoordEncoding(nn.Module):
    def __init__(
        ...
        self.embed_dim = embed_dim
        self.embed_x = nn.Embedding(grid_size_x, embed_dim)
        self.embed_y = nn.Embedding(grid_size_y, embed_dim)
    
    def forward(self, x_coor, y_coor):
        x_embed = self.embed_x((x_coor*self.grid_size_x).long())
        y_embed = self.embed_y((y_coor*self.grid_size_y).long())
        pos_enc = x_embed + y_embed
        return pos_enc
