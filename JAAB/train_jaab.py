...
from dataset.frame import ActionSpotFocusVideoDataset
...
def main(args):
    ...
    wandb.init(config = args, dir = args.save_dir + '/wandb_logs', project = 'TDEEDEXP', name = args.model + '-' + str(args.seed))
        ...
        for epoch in range(epoch, num_epochs):
            ...
            if args.criterion == 'loss':
                ...
                if epoch >= args.start_val_epoch:
                    time_map0 = time.time()
                    val_mAP = evaluate(model, val_data_frames, 'VAL', classes, printed=False, test=False)
                    time_map1 = time.time()
    ...
    for split in eval_splits:
        ...
        if os.path.exists(split_path):
            split_data = ActionSpotFocusVideoDataset(
            ...
            mAPs, tolerances = evaluate(model, split_data, split.upper(), classes, pred_file, printed = True, 
                        test = True, augment = (args.dataset != 'soccernet') & (args.dataset != 'soccernetball') & (args.dataset != 'focus'))
